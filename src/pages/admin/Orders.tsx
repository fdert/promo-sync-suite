...
${companyName}`;
    ...
    try {
      ...
      if (customerWhatsapp) {
        ...
        try {
          ...
          if (notificationError || (notificationResult && (notificationResult as any).success === false)) {
            console.warn('Fallback to direct webhook send...');
            // جلب أول ويب هوك outgoing نشط
            ...
            if (outgoing?.webhook_url) {
              ...
              console.warn('No active outgoing webhook found for direct send');
            }
          }
  ...
  const updateOrder = async () => {
    ...
    if (!newOrder.customer_id || !newOrder.estimated_delivery_time) {
      toast({
        title: "خطأ",
        description: "يرجى ملء جميع الحقول المطلوبة (العميل والوقت التقريبي)",
        variant: "destructive",
      });
      return;
    }
  ...
  const openEditOrderDialog = (order: Order) => {
    ...
    setNewOrder({
      customer_id: order.customer_id || '',
      service_id: order.service_type_id || '',
                  ...
                  <div className="mt-4 p-3 bg-white rounded border">
                    <div className="grid grid-cols-3 gap-4 text-center">
                      <div>
                        ...
                        <p className="text-lg font-bold text-blue-600">{Number(selectedOrderForPayment.total_amount || 0).toLocaleString()} ر.س</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-lg font-bold text-red-600">{Number((selectedOrderForPayment.remaining_amount ?? ((selectedOrderForPayment.total_amount || 0) - (selectedOrderForPayment.paid_amount || 0)))).toLocaleString()} ر.س</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <>
                ...
                <div className="p-4 bg-muted rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      <p><strong>المبلغ الإجمالي:</strong> {Number(selectedOrderForInvoice.total_amount || 0).toLocaleString()} ر.س</p>
                      <p><strong>المبلغ المدفوع:</strong> {Number(selectedOrderForInvoice.paid_amount || 0).toLocaleString()} ر.س</p>
                      <p><strong>المبلغ المتبقي:</strong> {Number((selectedOrderForInvoice.remaining_amount ?? ((selectedOrderForInvoice.total_amount || 0) - (selectedOrderForInvoice.paid_amount || 0)))).toLocaleString()} ر.س</p>
                    </div>
                  </div>
      ...
      <Dialog open={isNewOrderDialogOpen} onOpenChange={setIsNewOrderDialogOpen}>
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 p-1">
            {/* الصف الأول: العميل، نوع الخدمة، الأولوية */}
            ...
            {/* الصف الثاني: تاريخ التسليم، الوقت التقريبي، وصف الطلب */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="remaining_amount"
                    type="text"
                    value={(newOrder.amount - newOrder.paid_amount).toFixed(2)}
                    disabled
                    className="bg-muted"
                  />
                </div>
            ...
            {/* إرفاق ملفات */}
            <div className="space-y-2">
              <Label htmlFor="attachments">إرفاق ملفات (اختياري)</Label>
              <Input
                id="attachments"
                type="file"
                multiple
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-2 justify-end pt-4 border-t">
              ...
              <Button 
                onClick={updateOrder}
                disabled={loading || !newOrder.customer_id || !newOrder.estimated_delivery_time}
              >
