-- إصلاح خطأ "evaluation_token is ambiguous" في دالة التقييم
...
BEGIN
    ...
    IF NEW.status = 'مكتمل' AND (OLD.status IS NULL OR OLD.status != 'مكتمل') THEN
        ...
        IF evaluation_id IS NULL THEN
            ...
            RAISE LOG 'تم إنشاء تقييم جديد برمز: % والمعرف: %', token_value, evaluation_id;
        ELSE
            RAISE LOG 'يوجد تقييم مسبق بالمعرف: %', evaluation_id;
            -- استخدام التقييم الموجود
            SELECT e.evaluation_token INTO token_value FROM evaluations e WHERE e.id = evaluation_id;
        END IF;
