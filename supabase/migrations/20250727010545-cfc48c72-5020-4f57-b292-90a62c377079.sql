-- إضافة حساب الشبكة بشكل آمن (فقط إذا لم يكن موجوداً)
DO $$
DECLARE
  network_account_exists boolean;
BEGIN
  -- التحقق من وجود حساب الشبكة
  SELECT EXISTS(
    SELECT 1 FROM accounts 
    WHERE account_name = 'الشبكة' AND account_type = 'أصول'
  ) INTO network_account_exists;
  
  -- إضافة الحساب فقط إذا لم يكن موجوداً
  IF NOT network_account_exists THEN
...
-- إعادة إنشاء القيود المحاسبية للمدفوعات المرتبطة بالشبكة
DO $$
DECLARE
  payment_rec RECORD;
  customer_name text;
