-- إصلاح مشكلة حقل rating في جدول evaluations
-- إزالة القيد المرفوض وتعديل الحقل ليقبل NULL
ALTER TABLE evaluations DROP CONSTRAINT IF EXISTS evaluations_rating_check;

-- تعديل العمود ليقبل NULL كقيمة افتراضية
ALTER TABLE evaluations ALTER COLUMN rating DROP NOT NULL;
ALTER TABLE evaluations ALTER COLUMN rating SET DEFAULT NULL;

-- تحديث دالة send_evaluation_link_on_completion لتستخدم NULL بدلاً من 0
    ...
    IF NEW.status = 'مكتمل' AND (OLD.status IS NULL OR OLD.status != 'مكتمل') THEN
        ...
        IF customer_record.whatsapp_number IS NOT NULL AND customer_record.whatsapp_number != '' THEN
            ...
            IF NOT EXISTS (SELECT 1 FROM evaluations WHERE order_id = NEW.id) THEN
                ...
                -- إدراج سجل تقييم جديد بدون rating (NULL)
...
-- إصلاح دالة advanced_order_notification بإزالة المراجع للحقول المفقودة
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' AND OLD.status != NEW.status THEN
        ...
        IF template_name IS NOT NULL THEN
            -- إعداد بيانات الطلب (بدون paid_amount)
