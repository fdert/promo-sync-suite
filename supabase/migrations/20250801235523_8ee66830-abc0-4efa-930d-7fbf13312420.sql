-- إصلاح trigger إرسال الإشعارات لاستخدام pg_net بدلاً من net
DROP TRIGGER IF EXISTS order_notification_trigger ON orders;
DROP FUNCTION IF EXISTS send_order_notification();

-- Create the updated notification function using pg_net
...
BEGIN
    ...
    SELECT INTO request_id net.http_post(
        ...
        headers := jsonb_build_object(
            ...
            'Authorization', 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdjdXFmeGFjbmJ4ZGxkc2JtZ3ZmIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MzQ3ODUyNCwiZXhwIjoyMDY5MDU0NTI0fQ.Rw2oTfR9KfCZBN17Pj0q0c6XEjBKn0jH3_6dkB3hKrs'
        ),
    ...
    RAISE LOG 'WhatsApp notification request sent: type=%, order_id=%, request_id=%', 
        notification_type, COALESCE(NEW.id, OLD.id), request_id;
