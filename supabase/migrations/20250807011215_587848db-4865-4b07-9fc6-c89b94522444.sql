-- إصلاح مشكلة webhook_url الغامض في دالة إرسال رسالة التقييم
    ...
    IF NEW.status = 'مكتمل' AND (OLD.status IS NULL OR OLD.status != 'مكتمل') THEN
        ...
        IF customer_record.whatsapp_number IS NOT NULL AND customer_record.whatsapp_number != '' THEN
            ...
            IF google_settings.place_id IS NOT NULL THEN
                ...
                -- جلب webhook التقييم
                SELECT ws.webhook_url INTO selected_webhook_url
                ...
                IF selected_webhook_url IS NULL THEN
                    SELECT ws.webhook_url INTO selected_webhook_url
                    FROM webhook_settings ws
                ...
                IF selected_webhook_url IS NOT NULL THEN
                    ...
                    RAISE LOG 'إرسال للـ webhook: %', selected_webhook_url;
                    RAISE LOG 'البيانات: %', payload::text;
                    
                    -- إرسال للـ webhook
                    PERFORM net.http_post(
                        url := selected_webhook_url,
