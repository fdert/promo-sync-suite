-- إضافة حالة 'queued' إلى constraint
ALTER TABLE bulk_campaign_messages 
DROP CONSTRAINT bulk_campaign_messages_status_check;

ALTER TABLE bulk_campaign_messages 
ADD CONSTRAINT bulk_campaign_messages_status_check 
CHECK (status = ANY (ARRAY['pending'::text, 'sent'::text, 'failed'::text, 'delivered'::text, 'queued'::text]));

-- تحديث دالة معالجة وإرسال الحملات لتستخدم 'sent' بدلاً من 'queued'
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            -- تحديث حالة الرسالة في الحملة إلى 'sent'
