...
-- تحديث دالة مزامنة أرصدة العملاء المدينون للعمل مع التريجر
CREATE OR REPLACE FUNCTION sync_accounts_after_order_change()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE TRIGGER sync_accounts_after_order_payment
    ...
    FOR EACH ROW
    EXECUTE FUNCTION sync_accounts_after_order_change();