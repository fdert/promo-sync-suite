-- تحديث حالة الرسائل من sent إلى pending لإعادة المعالجة
UPDATE whatsapp_messages 
SET status = 'pending'
WHERE id IN ('96706185-7f55-4d36-9872-2ad95be59a3c', '2d4561cd-5b2c-4d55-b469-54dc22a07123')
AND status = 'sent';

-- تحديث دالة معالجة الحملات لعدم تحديث الحالة إلى sent فوراً
...
BEGIN
    ...
    -- نسخ الرسائل إلى جدول whatsapp_messages للإرسال الفعلي مع الحفاظ على حالة pending
    ...
    LOOP
        BEGIN
            ...
            INSERT INTO whatsapp_messages (
                ...
                'pending',  -- ترك الحالة كـ pending ليتم معالجتها بواسطة Edge Function
                message_record.customer_id,
                now()
            );
            
            -- تحديث حالة الرسالة في الحملة إلى 'queued' للإشارة إلى أنها أُضيفت لقائمة الانتظار
    ...
    -- تحديث إحصائيات الحملة - تركها كـ sending حتى يتم الإرسال الفعلي
    UPDATE bulk_campaigns 
    SET 
        total_recipients = message_count,
        sent_count = 0,  -- سيتم التحديث لاحقاً عند الإرسال الفعلي
        failed_count = messages_failed,
        status = CASE WHEN messages_sent > 0 THEN 'processing' ELSE 'failed' END
    WHERE id = campaign_id_param;
    ...
    result := jsonb_build_object(
        ...
        'failed_count', messages_failed,
        'message', CASE 
            WHEN messages_sent > 0 THEN 'تم إضافة الرسائل لقائمة الانتظار'
