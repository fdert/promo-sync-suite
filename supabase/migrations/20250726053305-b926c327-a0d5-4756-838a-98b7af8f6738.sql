-- إصلاح دالة تحديث إحصائيات التقييم
CREATE OR REPLACE FUNCTION public.update_evaluation_stats()
RETURNS TRIGGER AS $$
BEGIN
  -- حذف الإحصائيات القديمة فقط إذا كانت موجودة
  DELETE FROM public.evaluation_stats WHERE id IS NOT NULL;
  
  -- إضافة إحصائيات جديدة فقط إذا كانت هناك تقييمات مرسلة
  ...
  SELECT 
    ...
    ROUND((COUNT(CASE WHEN would_recommend = true THEN 1 END)::numeric / NULLIF(COUNT(*), 0)::numeric) * 100, 2) as recommendation_percentage,
...
DROP TRIGGER IF EXISTS update_evaluation_stats_trigger ON public.evaluations;
CREATE TRIGGER update_evaluation_stats_trigger
    AFTER INSERT OR UPDATE OR DELETE ON public.evaluations
    FOR EACH STATEMENT
    EXECUTE FUNCTION public.update_evaluation_stats();