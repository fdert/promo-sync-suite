-- تحديث دالة إنشاء التقييم عند الاكتمال لإرسال رابط خرائط جوجل
...
BEGIN
    ...
    IF NEW.status = 'مكتمل' AND (OLD.status IS NULL OR OLD.status != 'مكتمل') THEN
        ...
        -- إرسال رسالة واتساب مع رابط خرائط جوجل
        -- جلب بيانات العميل
        ...
        IF customer_record.whatsapp_number IS NOT NULL AND customer_record.whatsapp_number != '' THEN
            ...
            IF google_settings.place_id IS NOT NULL THEN
                ...
                -- إنشاء محتوى رسالة التقييم مع رابط خرائط جوجل
                ...
                IF evaluation_webhook_url IS NOT NULL THEN
                    ...
                    RAISE LOG 'إرسال رسالة تقييم خرائط جوجل للـ webhook: %', evaluation_webhook_url;
                    ...
                    RAISE LOG 'تم إرسال رسالة تقييم خرائط جوجل بنجاح';
                ELSE
